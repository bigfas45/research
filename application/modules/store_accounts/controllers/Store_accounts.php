...
{
    // return the addrss of the shopper
...
function _make_sure_delete_allowed($update_id)
{
    ...
    // do not allow delete if shopper has item in basket 
    ...
    if ($num_rows > 0) {
        ...
        $num_rows = $query->num_rows();
         if ($num_rows > 0) {
         return FALSE; //delete must have made a purchase
...
function delete($update_id){
    if (!is_numeric($update_id)) {
        ...
        if ($allwoed == FALSE) {
                     $flash_msg = "You are not allwoed to delete this account";
...
function _generate_token($update_id)
{
  ...
  $last_six = substr($pword, $start_point,6);

  if (($pword_length > 5) AND ($last_login>0)) {
       $token = $last_six.$date_made.$last_login;
  }else{
    $token ='';
  }
  return $token;
...
{
$last_six = substr($token, 0 , 6); //last six from stored pword
...
$query = $this->db->query($sql, array($date_made, '%'. $last_six, $last_login));
...
function _get_customer_name($update_id, $optional_customer_data=NULL)
{
    if (!isset($optional_customer_data)) {
        ...
        // echo $optional_customer_data['lastname'];die();
       ...
       if ($submit == "Submit")
        {
                   ...
                   $this->form_validation->set_rules('username', 'User Name', 'required|max_length[240]'); 
                    ...
                    if ($this->form_validation->run() == TRUE) {
                        ...
                        if (is_numeric($update_id)) {
                            ...
                            $this->_update($update_id, $data);
                            $flash_msg = "The account details was successfully updated";
                            $value  = '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                                $this->session->set_flashdata('account', $value); 
                            redirect('store_accounts/create/'.$update_id);
                ...
                if (!is_numeric($update_id))  {
                    $data['headline'] = "Add New accounts";
                }else{
                    $data['headline'] = "update accounts Details";
                }
                $data['update_id'] = $update_id;
                $data['flash'] = $this->session->flashdata('account');
