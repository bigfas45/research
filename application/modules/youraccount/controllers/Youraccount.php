...
function test1()
{
 ...
 echo "The session variable was set";
  ...
  echo anchor('youraccount/test3','unset the session variable'). "<br>";
}
...
function test3()
{
  ...
  echo "The session value1 was unset";
  echo "<hr>";
  echo anchor('youraccount/test1','set the session variable'). "<br>";
  echo anchor('youraccount/test2 ','Get(display) the session variable'). "<br>";
...
function submit_login()
{
    ...
    $this->form_validation->set_rules('pword', 'Password', 'required|min_length[7]|max_length[35]');
       ...
       $query = $this->store_accounts->get_with_double_condition($col1, $value1, $col2, $value2) ;
           ...
           $remeber = $this->input->post('remeber', TRUE);
           if ($remeber == "remeber-me") {
             $login_type = "longterm";
           } else{
...
{
  // NOTE: the login type can be longter or shortterm
  if ($login_type == "longterm") {
  // set a cookie
    $this->load->module('site_cookies');
     $this->site_cookie->_set_cookie($user_id);
  ...
  // attempt to update cart and divert cart
  $this->_attempt_cart_divert($user_id);

    // sending user $user_id to the private area
...
function _attempt_cart_divert($user_id)
{
  ...
  $customer_session_id = $this->session->session_id;
   ...
   $num_rows = $query->num_rows();
   if ($num_rows > 0) {
     # There are records That need correction
...
function submit()
{
    ...
    if ($submit == "Submit")
        ...
        # process the form
     $this->load->library('form_validation');
    $this->form_validation->set_rules('username', 'User Name', 'required|min_length[5]|max_length[60]'); 
    $this->form_validation->set_rules('email', 'E-mail', 'required|valid_email|max_length[240]');
    $this->form_validation->set_rules('pword', 'Password', 'required|min_length[7]|max_length[35]');
