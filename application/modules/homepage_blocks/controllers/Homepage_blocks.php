...
$flash_msg = "The offer block entry  was successful deleted";
...
function _draw_blocks()
{
	...
	$data['query'] = $this->db->query("SELECT * FROM `store_categories` WHERE parent_cart_id = '0' AND `status`='1' ");
...
function view_more()
{
	 ...
	 $data['query'] = $this->db->query("SELECT * FROM `store_categories` WHERE `parent_cart_id`='$find_id'");
...
function _draw_blocks_researcher()
{
	// draw the offer blocks that are on the homepage
	$data['query'] = $this->get('priority');
	$num_rows = $data['query']->num_rows();
	if ($num_rows>0) {
		$this->load->view('homepage_blocks_researcher', $data);
	}
       ...
       if ($submit == "Submit")
        {
					...
					if ($this->form_validation->run() == TRUE) {
						...
						if (is_numeric($update_id)) {
							...
							$this->_update($update_id, $data);
							$flash_msg = "The homepage  details was successfully updated";
							$value  = '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
							...
							$flash_msg = "The homepage   was successfully added";
