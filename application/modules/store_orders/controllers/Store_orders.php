 ...
 {
    // NOTE:notifies customer when an order staus has been updated
...
{
    // NOTE: this get called by / browse and it figuers out the order status title 
      $this->load->module('store_order_status');
...
function submit_order_status()
{
    ...
    if ($submit == "Cancel") {
      // get current order status for this order
        ...
        $this->_send_auto_notify($update_id);//notifu=y the customer of this update

        $flash_msg = "The Order status  was successful updated";
...
function browse()
{
    ...
    // count the orders that belongs to the order status
    $use_limit = FALSE;
...
function _get_mc_gross($paypal_id)
{
    ...
    $mc_gross = $posted_information['amount_1'];
}
return $mc_gross;
...
{
// this get called from the paypal ipn listerner when an order is placed
    $this->load->module('site_security');
    ...
    // transfer from storebasket to store shopper track
    $this->load->module('store_shoppertrack');
    $this->store_shoppertrack->_tranfer_from_basket($customer_session_id);
