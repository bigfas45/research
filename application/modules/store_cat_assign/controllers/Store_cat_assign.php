	...
	function submit($item_id)
{
			...
			redirect('store_items/create/'.$item_id);
				...
				if ($cat_id !="") {
                    ...
                    $cat_tittle = $this->store_categories->_get_cat_title($cat_id);


					$flash_msg = "The Item Was successfully Assigned To The ".$cat_tittle." Category";
					$value = '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
					$this->session->set_flashdata('item', $value); 
				}
			}
...
// get an array of all sub categories on the site
	...
	foreach ($query->result() as $row) {
	$cat_tittle = $this->store_categories->_get_parent_cat_title($row->cat_id); 

	$parent_cat_title = $this->store_categories->_get_parent_cat_title($row->cat_id); 
		$assigned_categories[$row->cat_id] = $parent_cat_title." > ".$cat_tittle;
	}
	...
	if (!isset($assigned_categories)) {
		...
		// the item has been assign to at least on cat
